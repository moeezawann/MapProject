...
fetch(
  "https://public.opendatasoft.com/api/explore/v2.1/catalog/datasets/us-county-boundaries/records?limit=100&refine=stusab%3A%22NC%22"
)
  .then((res) => res.json())
  .then((data) => {
     ...
     // in NC with names
      L.geoJSON(geojsonFeatures, {
        onEachFeature: function (feature, layer) {
          const matchingCountyData = processedData.find(
            (data) => data.county === feature.properties.HQ_NAME
          );
...
function addMedConditionToMap(conditionId, conditionName) {
    let temp_stack = countyCoordinates.splice();
          ...
          if (matchingCountyData && filteredData) {
            ...
            if (CancerNames.includes(conditionId)) {
              tooltipContent = `<strong>${matchingCountyData.county}</strong><br>
                        ...
                        <br>Total: ${matchingCountyData.health_data.cancer[conditionId].total.toLocaleString()}<br>`;
              layer.bindPopup(tooltipContent);
              feature.properties.total = matchingCountyData.health_data.cancer[conditionId].total;
             // feature.geometry.coordinates = stack.pop(0);
             // console.log(stack);
            //  console.log("printing----" +feature);
            //  style(feature);
              console.log(feature);
             // L.geoJson(feature, {style},).addTo(map)
            } else if (stiStdNames.includes(conditionId)) {
              tooltipContent = `<strong>${
                matchingCountyData.county
              }</strong><br>
                        Population: ${matchingCountyData.population.toLocaleString()}<br><br>
                        <strong>${conditionName}</strong>
                        <br>Total: ${matchingCountyData.health_data.stis_and_stds[
                          conditionId
              ...
              tooltipContent = `<strong>${
                matchingCountyData.county
                    ...
                    Population: ${matchingCountyData.population.toLocaleString()}<br><br>
                    <strong>${conditionName}</strong>
                    <br>Total: ${matchingCountyData.health_data[
                      conditionId
                    ].total.toLocaleString()}<br>`;
              layer.bindPopup(tooltipContent);
            }
